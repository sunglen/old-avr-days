...
// prepare for led-dot-matrix output.
...
int main(void)
{
	...
	lcd_gotoxy(0,0);
	lcd_puts("keys initialed.");
	mydelay(0xff);
	RTC_init(); /* init async RTC with 32kHz-Crystal */
	lcd_gotoxy(0,0);
	lcd_puts("RTC initialed.");
	mydelay(0xff);

	sei(); /* enable interrupts */

	/* these code will activate(?) PD7(OC2), and MAYBE interfer 
	   the T/C2 overflow interrupt. i don't know how!
	*/
	//	DDRA = 0xFF;
	//	DDRD = 0xFF;
