...
5V voltage will make the led-dot-matrix works terrible.
through a test, 3.0V will work better.(set the vtarg of stk500 to 2.7V or
use two AA alakine batteries.)
**************************************************************************/
...
int main(void)
{
	...
	{	
	  ...
	  PORTD = (RTC_h/10)|((((year%1000)%100)/10)<<4);
	  PORTB = ~0x01;
