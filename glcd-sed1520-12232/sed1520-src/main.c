...
{		
	...
	while (1) {
		...
		lcd_circle(90,20,10,LCD_MODE_SET);
		demo_upd_delay(DELAY);
		
		/* text and fonts */
		lcd_text(5,4,FONT_SIX_DOT,"#abcdefghijkl");
		demo_upd_delay(DELAY);
		lcd_text(5,20,FONT_SEVEN_DOT,"Martin");
		demo_upd_delay(DELAY);
		...
		/* fonts & inverse */
		lcd_erase();
		lcd_text(5,0,FONT_FIFTEEN_DOT,"Martin");
		lcd_text(5,16,FONT_FIFTEEN_DOT,"Thomas");
		demo_upd_delay(DELAY);
		...
		/* progmem strings */
		lcd_text_p(6,3,FONT_NINE_DOT,PSTR("Martin Thomas")); 
		lcd_text_P(6,15,FONT_SIX_DOT,"Software Engineering");
		demo_upd_delay(DELAY);
		demo_upd_delay(DELAY);
	}
