...
uint8_t glcd_put_hzxbm_16x16(PGM_P hz, uint8_t page, uint8_t col, uint8_t cs, uint8_t cement)
{
  ...
  if(col+15 > MAX_COLUMN)
    {
      if(cs==1 && cement){
	/*maybe left a too long space between two chip,
	  should be solved in later version.*/
	return glcd_put_hzxbm_16x16(hz, page, SCRN_LEFT, 2, 1);
	
      }else if(cs==2 && cement){
	return glcd_put_hzxbm_16x16(hz, page+1, SCRN_LEFT, 1, 1);
      }else{
	page++;
	col = SCRN_LEFT;
      }
...
/*
  todo:
  write a function that can display a string of chinese characters,
  which defined by
  
  include "hzk.h"
  
  PGM_P s[] PROGMEM = {
  sun_bits, ge_bits, shi_bits, ge_bits, bai_bits, chi_bits};
