...
for SD connection, see mmc.h for details.
*/
  ...
  if(col+15 > MAX_COLUMN)
    {
      if(cs==2 && cement){
	//hz16x16 need 2 pages.
	return glcd_putc_hz16x16(hz, page+2, SCRN_LEFT, 1, 1);
      }else if(!cement){
	page=page+2;
	col = SCRN_LEFT;
      }
...
uint8_t glcd_puts_hz(PGM_P s, int len, uint8_t page, uint8_t col, uint8_t cs, uint8_t cement)
{
  ...
  for(i=0;i<len;i++){
    ...
    hz_high=pgm_read_byte(s+i*2);
    hz_low=pgm_read_byte(s+i*2+1);
    
    //this offset is accordance with 32 bytes block size.
    offset=(hz_high-0xa1)*94+(hz_low-0xa1);
    //set block size to 32 bytes.
    mmc_read_block(offset, buffer, 32);
    
    current_col=glcd_putc_hz16x16(buffer, page, col, cs, cement);
