...
const unsigned char auc_frequencyL[4] PROGMEM = {
  23,25,
  27,31};
...
const unsigned char auc_frequencyH[4] PROGMEM = {
40,44,
48,54};
...
{ 
  if (x_SWa == 0 || x_SWb == 0){
    //set OCR1A to half width to show the pwm base clearly.
    //to set timebase=1us or 5us of oszifox to display pwm base wave clearly.
    //and this way can avoid noise.
    OCR1A = 127;

    // set OCR1A to top will output continuous High in OC1A.
    //OCR1A = 255;
  }else{
...
int main (void)
{
  ...
  for(;;){ 
	...
	break;
      case KEY1:
	x_SWb = pgm_read_byte(auc_frequencyL+0);
        x_SWa = pgm_read_byte(auc_frequencyH+0);
	break;
      case KEY2:
	x_SWb = pgm_read_byte(auc_frequencyL+0);
        x_SWa = pgm_read_byte(auc_frequencyH+1);
	break;
      case KEY3:
	x_SWb = pgm_read_byte(auc_frequencyL+0);
        x_SWa = pgm_read_byte(auc_frequencyH+2);
	break;
      case KEY4:
	x_SWb = pgm_read_byte(auc_frequencyL+0);
        x_SWa = pgm_read_byte(auc_frequencyH+3);
	break;
      case KEY5:
	x_SWb = pgm_read_byte(auc_frequencyL+1);
        x_SWa = pgm_read_byte(auc_frequencyH+0);
	break;
      case KEY6:
	x_SWb = pgm_read_byte(auc_frequencyL+1);
	...
	break;
      case KEY9:
	x_SWb = pgm_read_byte(auc_frequencyL+2);
        x_SWa = pgm_read_byte(auc_frequencyH+0);
	break;
      case KEY10:
	x_SWb = pgm_read_byte(auc_frequencyL+2);
	...
	break;
      case KEY13:
	x_SWb = pgm_read_byte(auc_frequencyL+3);
        x_SWa = pgm_read_byte(auc_frequencyH+0);
	break;
      case KEY14:
	x_SWb = pgm_read_byte(auc_frequencyL+3);
