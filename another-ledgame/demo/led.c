...
/*---------------------------------------------------------------------
Notes:
Some changes have been applied in order to fit my
hardware other than which was mentioned above. 
Please see http://sunge.awardspace.com for details.
Sun Ge
---------------------------------------------------------------------*/
#include <avr/io.h>
#include "led.h"
...
void led_disp(void)
{
	...
	if(scanflag){
		...
		led_sendword(~ledvram[(int)row]);	// カソード	
	}else{
		led_sendword(0xFFFF);	
		led_sendwordv(temp);	//アノード
